...
(function () {
  ...
  function getReferrerLabel(code) {
    if (!code) return '-';
    const mapping = {
      aj001: '추천코드 01', aj002: '추천코드 02', aj003: '추천코드 03', aj004: '추천코드 04', aj005: '추천코드 05',
      ab001: '추천코드 06', ab002: '추천코드 07', ab003: '추천코드 08', ab004: '추천코드 09', ab005: '추천코드 10',
      ac001: '추천코드 11', ac002: '추천코드 12', ac003: '추천코드 13', ac004: '추천코드 14', ac005: '추천코드 15',
      as001: '추천코드 16', as002: '추천코드 17', as003: '추천코드 18', as004: '추천코드 19',
      cs020: '추천코드 20',
    };
  ...
  const __saveBlacklistOriginal = saveBlacklist;
  const __saveBlacklistPending = new Map();
  saveBlacklist = async function saveBlacklistLocked(userId, enable, payload = {}) {
    if (__saveBlacklistPending.has(userId)) {
      return __saveBlacklistPending.get(userId);
    }

    const task = __saveBlacklistOriginal(userId, enable, payload);
    __saveBlacklistPending.set(userId, task);
    try {
      return await task;
    } finally {
      __saveBlacklistPending.delete(userId);
    }
  };
