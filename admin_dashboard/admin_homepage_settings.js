...
(function () {
  ...
  async function compressImageDataUrl(dataUrl, options = {}) {
    ...
    if (originalBytes && originalBytes < 120 * 1024) {
      return dataUrl;
    }
    ...
    const blob = await new Promise((resolve) => {
      canvas.toBlob((result) => resolve(result), 'image/webp', settings.quality);
    });
  ...
  function getCompressionConfig(section) {
    return IMAGE_COMPRESSION[section] || IMAGE_COMPRESSION.main;
  }
  ...
  async function applyMediaFile(section, file) {
    ...
    const compressedUrl = await compressImageDataUrl(dataUrl, getCompressionConfig(section));
    const meta = await readImageMeta(compressedUrl);
    setValue(cfg.valueId, compressedUrl);
    ...
    if (sizeHint) {
      ...
      sizeHint.textContent = `${dimensionText} · ${formatBytes(estimateDataUrlBytes(compressedUrl) || file.size)} · ${cfg.recommended}`;
    }
  }
  ...
  async function applyBannerImage(item, section, file) {
    ...
    if (hiddenInput) hiddenInput.value = compressedUrl;
    if (preview) {
      preview.src = compressedUrl;
      preview.style.display = 'block';
    }
    ...
    if (sizeHint) {
      const dimensionText = meta ? `${meta.width} × ${meta.height}px` : '이미지 업로드됨';
      sizeHint.textContent = `${dimensionText} · ${formatBytes(estimateDataUrlBytes(compressedUrl) || file.size)} · ${config.recommended}`;
    }
    if (metaLabel) {
      ...
      if (type === 'homepage') {
        ...
        payload.banners = await Promise.all(
          serializeBannerSection('main').map(async (banner) => ({
            ...banner,
            imgUrl: await compressImageDataUrl(banner.imgUrl, getCompressionConfig('main')),
          })),
        );
        payload.bottom_banners = await Promise.all(
          serializeBannerSection('bottom').map(async (banner) => ({
            ...banner,
            imgUrl: await compressImageDataUrl(banner.imgUrl, getCompressionConfig('bottom')),
          })),
        );
      } else if (type === 'footer') {
