...
// .wrangler/tmp/bundle-ZUN7Or/checked-fetch.js
...
// .wrangler/tmp/pages-sUr7xg/functionsWorker-0.5124223222267719.mjs
...
__name2(normalizeOrigin, "normalizeOrigin");
...
var recommendationsSchemaReady = false;
var classesSchemaReady = false;
var reviewsSchemaReady = false;
var classStatsSchemaReady = false;
var dmMessagesSchemaReady = false;
var userChatsSchemaReady = false;
var contactsSchemaReady = false;
var groupChatsSchemaReady = false;
var chatMessagesSchemaReady = false;
var gatheringsSchemaReady = false;
var operationsSchemaReady = false;
var siteSettingsSchemaReady = false;
...
__name2(ensureRecommendationsSchema, "ensureRecommendationsSchema");
...
__name2(ensureClassesSchema, "ensureClassesSchema");
...
__name2(ensureReviewsSchema, "ensureReviewsSchema");
...
__name2(ensureClassStatsSchema, "ensureClassStatsSchema");
...
__name2(ensureDmMessagesSchema, "ensureDmMessagesSchema");
...
__name2(ensureUserChatsSchema, "ensureUserChatsSchema");
...
__name2(ensureContactsSchema, "ensureContactsSchema");
...
__name2(ensureGroupChatsSchema, "ensureGroupChatsSchema");
...
__name2(ensureChatMessagesSchema, "ensureChatMessagesSchema");
...
__name2(ensureGatheringsSchema, "ensureGatheringsSchema");
...
__name2(ensureOperationsSchema, "ensureOperationsSchema");
...
__name2(ensureSiteSettingsSchema, "ensureSiteSettingsSchema");
...
__name2(isMasterAdminUserId, "isMasterAdminUserId");
...
__name2(applyMasterAdminOverride, "applyMasterAdminOverride");
...
async function requireClassManager(context, classId) {
  ...
  if (isAtLeastRole(current.user.role, "operator") || cls.creator_id === userId || hasSubInstructorAccess(cls.sub_instructors, userId)) {
    return current;
  }
  return {
    ok: false,
...
__name2(requireClassManager, "requireClassManager");
function hasSubInstructorAccess(rawValue, userId) {
  const targetId = String(userId || "").trim();
  if (!targetId) return false;
  const matches = /* @__PURE__ */ __name2((value) => String(value || "").trim() === targetId, "matches");
  ...
  return rawText.split(/[,\s]+/).map((item) => item.trim()).filter(Boolean).some(matches);
}
__name(hasSubInstructorAccess, "hasSubInstructorAccess");
__name2(hasSubInstructorAccess, "hasSubInstructorAccess");
var classCategoriesSchemaReady = false;
var classBookmarksSchemaReady = false;
var classCategoriesSeeded = false;
var DEFAULT_CLASS_CATEGORIES = [
  { name: "\uC18C\uBAA8\uC784/\uB3D9\uC544\uB9AC", emoji: "\u{1F465}", sort_order: 10 },
  { name: "\uB9DB\uC788\uB294 \uD074\uB798\uC2A4", emoji: "\u{1F37D}\uFE0F", sort_order: 20 },
  { name: "\uC6B4\uB3D9 \uD074\uB798\uC2A4", emoji: "\u{1F3CB}\uFE0F", sort_order: 30 },
  { name: "\uB514\uC790\uC778", emoji: "\u{1F3A8}", sort_order: 40 },
  { name: "\uC0DD\uC0B0\uC131", emoji: "\u26A1", sort_order: 50 },
  { name: "\uC2A4\uD3EC\uCE20", emoji: "\u{1F3C5}", sort_order: 60 },
  { name: "\uB514\uC9C0\uD138 \uB4DC\uB85C\uC789", emoji: "\u270F\uFE0F", sort_order: 70 },
  { name: "\uC131\uACF5 \uB9C8\uC778\uB4DC", emoji: "\u{1F9E0}", sort_order: 80 },
  { name: "\uC74C\uC545", emoji: "\u{1F3B5}", sort_order: 90 },
  { name: "\uC694\uB9AC", emoji: "\u{1F373}", sort_order: 100 },
  { name: "\uBCA0\uC774\uD0B9", emoji: "\u{1F9C1}", sort_order: 110 },
  { name: "\uC0AC\uC9C4", emoji: "\u{1F4F7}", sort_order: 120 },
  { name: "\uC601\uC0C1", emoji: "\u{1F3AC}", sort_order: 130 },
  { name: "\uACF5\uC608", emoji: "\u{1F9F5}", sort_order: 140 },
  { name: "\uC5EC\uD589", emoji: "\u{1F9ED}", sort_order: 150 }
];
var CATEGORY_EMOJI_MAP = new Map(DEFAULT_CLASS_CATEGORIES.map((item) => [item.name, item.emoji]));
function normalizeCategoryName(value) {
  return String(value ?? "").trim();
}
__name(normalizeCategoryName, "normalizeCategoryName");
__name2(normalizeCategoryName, "normalizeCategoryName");
function defaultCategoryEmoji(name) {
  return CATEGORY_EMOJI_MAP.get(normalizeCategoryName(name)) || "\u2728";
}
__name(defaultCategoryEmoji, "defaultCategoryEmoji");
__name2(defaultCategoryEmoji, "defaultCategoryEmoji");
function uniqueCategories(values) {
  return Array.from(
    new Set(
      values.map((value) => normalizeCategoryName(value)).filter(Boolean)
    )
  );
}
__name(uniqueCategories, "uniqueCategories");
__name2(uniqueCategories, "uniqueCategories");
...
__name(extractSubInstructorIds, "extractSubInstructorIds");
__name2(extractSubInstructorIds, "extractSubInstructorIds");
...
__name(buildNotInClause, "buildNotInClause");
__name2(buildNotInClause, "buildNotInClause");
function categorySortOrder(name, fallbackIndex) {
  const normalized = normalizeCategoryName(name);
  const match2 = DEFAULT_CLASS_CATEGORIES.find((item) => item.name === normalized);
  return match2 ? match2.sort_order : 1e3 + fallbackIndex;
}
__name(categorySortOrder, "categorySortOrder");
__name2(categorySortOrder, "categorySortOrder");
async function ensureClassCategoriesSchema(db) {
  await ensureClassesSchema(db);
...
__name(ensureClassCategoriesSchema, "ensureClassCategoriesSchema");
__name2(ensureClassCategoriesSchema, "ensureClassCategoriesSchema");
async function ensureClassBookmarksSchema(db) {
  await ensureClassesSchema(db);
...
__name(ensureClassBookmarksSchema, "ensureClassBookmarksSchema");
__name2(ensureClassBookmarksSchema, "ensureClassBookmarksSchema");
async function loadClassStaffIds(db, classId) {
...
__name(loadClassStaffIds, "loadClassStaffIds");
__name2(loadClassStaffIds, "loadClassStaffIds");
async function refreshClassStats(db, classId) {
  const id = String(classId || "").trim();
  if (!id) return null;
  await ensureClassesSchema(db);
...
__name(refreshClassStats, "refreshClassStats");
__name2(refreshClassStats, "refreshClassStats");
async function loadClassCategories(db, { activeOnly = true } = {}) {
...
__name(loadClassCategories, "loadClassCategories");
__name2(loadClassCategories, "loadClassCategories");
...
async function loadClassesByIds(db, classIds = [], { publicOnly = true } = {}) {
  await ensureClassesSchema(db);
  await ensureClassStatsSchema(db);
...
__name(loadClassesByIds, "loadClassesByIds");
__name2(loadClassesByIds, "loadClassesByIds");
function getEffectiveClassPrice(row) {
...
__name(getEffectiveClassPrice, "getEffectiveClassPrice");
__name2(getEffectiveClassPrice, "getEffectiveClassPrice");
function getClassHotScore(row) {
  ...
  return Math.round(
    bookmarks * 24 + reviews * 18 + avgRating * 32 + visits * 0.5 + enrollments * 10 + gatherings * 4
  );
}
__name(getClassHotScore, "getClassHotScore");
__name2(getClassHotScore, "getClassHotScore");
...
__name(uniqueStrings2, "uniqueStrings2");
__name2(uniqueStrings2, "uniqueStrings");
function buildNotInClause2(column, values = []) {
  const ids = uniqueStrings2(values);
  if (!ids.length) {
    return { clause: "", binds: [] };
  }
  return {
...
__name(buildNotInClause2, "buildNotInClause2");
__name2(buildNotInClause2, "buildNotInClause");
...
__name(normalizeSubInstructorItem, "normalizeSubInstructorItem");
__name2(normalizeSubInstructorItem, "normalizeSubInstructorItem");
async function loadSubInstructorProfiles(db, rawSubInstructors = []) {
  ...
  const ids = uniqueStrings2(refs.map((item) => {
    if (typeof item === "string") return item;
  ...
  return refs.map((item, index) => {
    const rawId = typeof item === "string" ? item : item && typeof item === "object" ? item.id || item.user_id || item.userId || "" : "";
    const profile = rawId ? byId.get(String(rawId).trim()) || {} : {};
    return normalizeSubInstructorItem(item, profile, index);
  });
}
__name(loadSubInstructorProfiles, "loadSubInstructorProfiles");
__name2(loadSubInstructorProfiles, "loadSubInstructorProfiles");
...
__name(loadRevenueByRange, "loadRevenueByRange");
__name2(loadRevenueByRange, "loadRevenueByRange");
async function loadStudentStats(db, classId, excludedIds = []) {
  const filter = buildNotInClause2("user_id", excludedIds);
  const row = await db.prepare(`
    SELECT
...
__name(loadStudentStats, "loadStudentStats");
__name2(loadStudentStats, "loadStudentStats");
...
__name(loadRefundSummary, "loadRefundSummary");
__name2(loadRefundSummary, "loadRefundSummary");
...
__name(loadRecentRefundLogs, "loadRecentRefundLogs");
__name2(loadRecentRefundLogs, "loadRecentRefundLogs");
async function loadMeetingStats(db, classId, excludedIds = []) {
  const filter = buildNotInClause2("gp.user_id", excludedIds);
...
__name(loadMeetingStats, "loadMeetingStats");
__name2(loadMeetingStats, "loadMeetingStats");
async function deleteClassReferences(db, classId) {
  ...
  const { results: folders } = await db.prepare("SELECT folder_id, class_ids, type FROM recommendations").all().catch(() => ({ results: [] }));
...
__name(deleteClassReferences, "deleteClassReferences");
__name2(deleteClassReferences, "deleteClassReferences");
async function onRequest(context) {
  ...
  try {
    ...
    if (request.method === "GET") {
      ...
      const instructorIds = uniqueStrings2([
      ...
      const avgRevenuePerMeeting = totalMeetings > 0 ? Math.round(totalRevenue / totalMeetings) : 0;
    ...
    if (request.method === "PATCH") {
      ...
      const nextIsPublic = body.is_public === void 0 ? null : body.is_public ? 1 : 0;
...
async function loadRevenueByRange2(db, classId) {
  const rangeDefs = {
...
__name(loadRevenueByRange2, "loadRevenueByRange2");
__name2(loadRevenueByRange2, "loadRevenueByRange");
...
__name(loadRanking, "loadRanking");
__name2(loadRanking, "loadRanking");
...
__name(loadCategorySummary, "loadCategorySummary");
__name2(loadCategorySummary, "loadCategorySummary");
async function loadDetail(db, classId) {
  ...
  const instructor = classRow.instructor_id ? await db.prepare("SELECT id, name, email, phone, profile_image_url, role FROM users WHERE id = ?").bind(classRow.instructor_id).first().catch(() => null) : null;
  ...
  const revenueByRange = await loadRevenueByRange2(db, classId);
  return {
    class: {
      ...classRow,
      image_urls: safeParseJSON2(classRow.image_urls, []),
      curriculum: safeParseJSON2(classRow.curriculum, []),
      sub_instructors: safeParseJSON2(classRow.sub_instructors, []),
      target_audience: safeParseJSON2(classRow.target_audience, []),
      objectives: safeParseJSON2(classRow.objectives, []),
      is_public: Number(classRow.is_public ?? 1) === 1,
      is_approved: Number(classRow.is_approved ?? 0) === 1
    },
    instructor,
...
__name(loadDetail, "loadDetail");
__name2(loadDetail, "loadDetail");
...
__name2(normalizeEmoji, "normalizeEmoji");
...
__name(propagateCategoryRename, "propagateCategoryRename");
__name2(propagateCategoryRename, "propagateCategoryRename");
...
__name(propagateCategoryDelete, "propagateCategoryDelete");
__name2(propagateCategoryDelete, "propagateCategoryDelete");
...
__name(fetchAllCategories, "fetchAllCategories");
__name2(fetchAllCategories, "fetchAllCategories");
async function onRequest3(context) {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      const isActive = body.is_active === void 0 ? 1 : body.is_active ? 1 : 0;
...
__name2(loadClasses, "loadClasses");
async function onRequest4(context) {
  ...
  try {
    ...
    const limit = Math.min(Math.max(parseIntSafe(url.searchParams.get("limit"), 500), 1), 1e3);
...
__name2(normalizeBoolean, "normalizeBoolean");
...
__name2(normalizeLimit, "normalizeLimit");
function normalizeOffset(value) {
  const num = Number(value);
  return Number.isFinite(num) && num > 0 ? Math.floor(num) : 0;
}
__name(normalizeOffset, "normalizeOffset");
__name2(normalizeOffset, "normalizeOffset");
...
__name(dedupeBySignature, "dedupeBySignature");
__name2(dedupeBySignature, "dedupeBySignature");
...
function blacklistSignature(row) {
  return [
    normalizeText2(row?.user_id),
    normalizeText2(row?.previous_state),
    normalizeText2(row?.new_state),
    normalizeText2(row?.changed_by),
    normalizeText2(row?.reason)
  ].join("|");
}
__name(blacklistSignature, "blacklistSignature");
__name2(blacklistSignature, "blacklistSignature");
function refundSignature(row) {
  return [
    normalizeText2(row?.user_id),
    normalizeText2(row?.order_id),
    normalizeText2(row?.class_id),
    normalizeText2(row?.refund_type),
    normalizeText2(row?.original_amount),
    normalizeText2(row?.refund_amount),
    normalizeText2(row?.reason_tags),
    normalizeText2(row?.reason_note),
    normalizeText2(row?.status),
    normalizeText2(row?.processed_by)
  ].join("|");
}
__name(refundSignature, "refundSignature");
__name2(refundSignature, "refundSignature");
async function onRequestGet(context) {
  ...
  try {
    ...
    const refundRows = await env.DB.prepare(`
      ...
      LIMIT ? OFFSET ?
    `).bind(limit, offset).all();
    const blacklist_logs_raw = (blacklistRows.results || []).filter((row) => row.previous_state !== row.new_state).map((row) => ({
      id: row.id,
      user: mapUser(row),
...
__name2(parseMaybeJsonArray, "parseMaybeJsonArray");
...
__name(uniqueStrings3, "uniqueStrings3");
__name2(uniqueStrings3, "uniqueStrings");
function extractClassIds(rawFolder) {
  ...
  return uniqueStrings3([...directClassIds, ...snakeClassIds, ...objectIds]);
}
__name(extractClassIds, "extractClassIds");
__name2(extractClassIds, "extractClassIds");
...
__name2(normalizeFolder, "normalizeFolder");
...
__name2(parseRequestTargetType, "parseRequestTargetType");
function parseDeletedFolderIds(body) {
  const raw = body?.deletedFolderIds ?? body?.removedFolderIds ?? body?.deletedIds ?? body?.removedIds ?? [];
  return uniqueStrings3(parseMaybeJsonArray(raw));
}
__name(parseDeletedFolderIds, "parseDeletedFolderIds");
__name2(parseDeletedFolderIds, "parseDeletedFolderIds");
async function enrichFolders(db, folders) {
  const classIds = Array.isArray(folders) ? folders.flatMap((folder) => parseMaybeJsonArray(folder.class_ids)) : [];
  ...
  return (folders || []).map((folder) => {
    ...
    const folderClasses = folderClassIds.map((id) => {
...
__name2(enrichFolders, "enrichFolders");
...
__name2(upsertFolders, "upsertFolders");
async function deleteFolders(db, type, folderIds) {
  const ids = uniqueStrings3(folderIds);
...
__name2(deleteFolders, "deleteFolders");
...
function normalizeTags(value) {
  ...
  if (typeof value === "string") {
    return value.split(",").map((item) => normalizeText3(item)).filter(Boolean);
  }
  return [];
}
__name(normalizeTags, "normalizeTags");
__name2(normalizeTags, "normalizeTags");
...
__name(buildRefundMemo, "buildRefundMemo");
__name2(buildRefundMemo, "buildRefundMemo");
async function onRequestPost(context) {
  ...
  try {
    ...
    const orderId = normalizeText3(body.order_id);
    const userId = normalizeText3(body.user_id);
    const refundType = normalizeText3(body.refund_type || "full").toLowerCase();
    const reasonTags = normalizeTags(body.reason_tags || body.tags);
    const reasonNote = normalizeText3(body.reason_note || body.reason || "");
    if (!orderId) {
    ...
    if (userId && normalizeText3(order.user_id) && normalizeText3(order.user_id) !== userId) {
    ...
    const originalAmount = toNumber2(order.final_amount || order.amount || 0);
    if (!originalAmount) {
      return json(request, env, { success: false, error: "\uD658\uBD88\uD560 \uAE08\uC561\uC774 \uC5C6\uC2B5\uB2C8\uB2E4." }, { status: 400 });
    }
    let refundAmount = originalAmount;
    if (refundType === "partial") {
      refundAmount = toNumber2(body.refund_amount);
    ...
    `).bind(
      refundId,
      normalizeText3(order.user_id || userId),
      order.order_id,
      order.class_id || null,
      order.class_title || "",
      refundType,
...
__name2(buildDateRange, "buildDateRange");
...
__name(rowsToMap, "rowsToMap");
__name2(rowsToMap, "rowsToMap");
...
__name2(generateCouponCode, "generateCouponCode");
async function ensureCouponSchema(db) {
  ...
  `).run();
  const addColumnIfMissing2 = /* @__PURE__ */ __name2(async (table, definition) => {
    try {
...
__name2(ensureCouponSchema, "ensureCouponSchema");
...
__name2(listCoupons, "listCoupons");
...
__name2(ensureMenuSchema, "ensureMenuSchema");
var DEFAULT_MENUS = [
...
__name2(parseSubInstructors, "parseSubInstructors");
...
__name2(buildBirthDate, "buildBirthDate");
...
__name2(buildClassSummary, "buildClassSummary");
...
__name2(loadOperators, "loadOperators");
...
__name2(canAssignRoleByCaller, "canAssignRoleByCaller");
  ...
  if (method === "POST") {
    try {
      ...
      `).bind(
        ...
        body.receipt_url || "",
        body.memo || "",
        isPaid ? (/* @__PURE__ */ new Date()).toISOString() : null
      ).run();
      ...
      if (isPaid && body.class_id) {
        await refreshClassStats(db, body.class_id).catch((error) => {
          console.warn("[API /admin/orders] refreshClassStats after order insert failed:", error.message);
        });
      }
...
__name2(ensureSettlementSchema, "ensureSettlementSchema");
...
__name(buildBirthDate2, "buildBirthDate2");
__name2(buildBirthDate2, "buildBirthDate");
...
__name(normalizeBoolean2, "normalizeBoolean2");
__name2(normalizeBoolean2, "normalizeBoolean");
...
__name2(sumMoney, "sumMoney");
function classKey(row) {
  return row?.class_id || row?.id || row?.title || "";
}
__name(classKey, "classKey");
__name2(classKey, "classKey");
function normalizeText4(value) {
  return String(value ?? "").trim();
}
__name(normalizeText4, "normalizeText4");
__name2(normalizeText4, "normalizeText");
function dedupeBySignature2(items, getSignature) {
  const seen = /* @__PURE__ */ new Set();
  const result = [];
  for (const item of items) {
    const signature = getSignature(item);
...
__name(dedupeBySignature2, "dedupeBySignature2");
__name2(dedupeBySignature2, "dedupeBySignature");
function blacklistSignature2(row) {
  return [
    normalizeText4(row?.user_id),
    normalizeText4(row?.previous_state),
    normalizeText4(row?.new_state),
    normalizeText4(row?.changed_by),
    normalizeText4(row?.reason)
  ].join("|");
}
__name(blacklistSignature2, "blacklistSignature2");
__name2(blacklistSignature2, "blacklistSignature");
function refundSignature2(row) {
  return [
    normalizeText4(row?.user_id),
    normalizeText4(row?.order_id),
    normalizeText4(row?.class_id),
    normalizeText4(row?.refund_type),
    normalizeText4(row?.original_amount),
    normalizeText4(row?.refund_amount),
    normalizeText4(row?.reason_tags),
    normalizeText4(row?.reason_note),
    normalizeText4(row?.status),
    normalizeText4(row?.processed_by)
  ].join("|");
}
__name(refundSignature2, "refundSignature2");
__name2(refundSignature2, "refundSignature");
...
__name2(safeQueryAll, "safeQueryAll");
...
__name2(safeQueryOne, "safeQueryOne");
async function loadMemberDetail(db, userId) {
  ...
  const refundLogs = dedupeBySignature2(refundRows.map((row) => ({
    id: row.id,
    user_id: row.user_id,
    order_id: row.order_id,
    class_id: row.class_id,
  ...
  const blacklistLogs = dedupeBySignature2(blacklistRows.map((row) => ({
    id: row.id,
    user_id: row.user_id,
    previous_state: normalizeBoolean2(row.previous_state),
    new_state: normalizeBoolean2(row.new_state),
    changed_by: row.changed_by || "",
    reason: row.reason || "",
    created_at: row.created_at || ""
  })), blacklistSignature2);
  return {
    user: {
      ...
      signup_date: user.created_at || "",
      is_blacklisted: normalizeBoolean2(user.is_blacklisted),
    ...
    instructor_classes: instructorClasses.map((row) => ({
      ...
      is_approved: normalizeBoolean2(row.is_approved),
      current_participants: Number(row.current_participants || 0),
      created_at: row.created_at || "",
      updated_at: row.updated_at || ""
    })),
...
__name2(loadMemberDetail, "loadMemberDetail");
...
async function onRequestPut2(context) {
  ...
  try {
    ...
    if (body.blacklisted !== void 0) {
      ...
      const nextBlacklisted = normalizeBoolean2(body.blacklisted);
      const currentBlacklisted = normalizeBoolean2(currentUser.is_blacklisted);
...
__name(bumpClassVisit, "bumpClassVisit");
__name2(bumpClassVisit, "bumpClassVisit");
...
__name(safeParseJSON3, "safeParseJSON3");
__name2(safeParseJSON3, "safeParseJSON");
async function onRequest15(context) {
  ...
  try {
    ...
    const chatCountResult = await db.prepare("SELECT COUNT(*) AS count FROM chat_messages WHERE class_id = ?").bind(classId).first().catch(() => ({ count: 0 }));
    ...
    const result = {
      ...
      image_urls: safeParseJSON3(classData.image_urls, []),
      curriculum: safeParseJSON3(classData.curriculum, []),
      sub_instructors: safeParseJSON3(classData.sub_instructors, []),
      target_audience: safeParseJSON3(classData.target_audience, []),
      objectives: safeParseJSON3(classData.objectives, []),
...
__name2(getPathParts, "getPathParts");
...
__name2(normalizeMessage, "normalizeMessage");
...
__name2(buildSinceClause, "buildSinceClause");
...
__name2(streamMessages, "streamMessages");
...
__name2(normalizeChatMessage, "normalizeChatMessage");
...
__name(refreshBookmarkCount, "refreshBookmarkCount");
__name2(refreshBookmarkCount, "refreshBookmarkCount");
async function onRequest17(context) {
  ...
  try {
    ...
    if (request.method === "GET") {
      const url = new URL(request.url);
      const classId2 = String(url.searchParams.get("class_id") || "").trim();
      if (!classId2) {
        return json(request, env, { success: false, error: "class_id is required" }, { status: 400 });
      }
      const exists = await db.prepare("SELECT 1 FROM class_bookmarks WHERE class_id = ? AND user_id = ?").bind(classId2, auth.user.id).first();
      const count2 = await db.prepare("SELECT COUNT(*) AS cnt FROM class_bookmarks WHERE class_id = ?").bind(classId2).first().catch(() => ({ cnt: 0 }));
      return json(request, env, {
        success: true,
        data: {
          bookmarked: !!exists,
          count: Number(count2?.cnt || 0)
        }
      });
    }
    if (request.method !== "POST") {
...
async function onRequestGet10(context) {
  ...
  try {
    ...
    return Response.json({ success: true, data: results }, { headers: RESPONSE_HEADERS2 });
  } catch (err) {
...
async function onRequestPost14(context) {
  ...
  try {
    ...
    await refreshClassStats(env.DB, finalClassId).catch((error) => {
      console.warn("[API] refreshClassStats after enrollment failed:", error.message);
    });
...
async function onRequestGet15(context) {
  ...
  try {
    ...
    return json(request, env, { success: true, data: results }, { headers: RESPONSE_HEADERS3 });
  } catch (err) {
...
__name2(ensureFriendsTable, "ensureFriendsTable");
...
async function onRequestGet18(context) {
  ...
  try {
    ...
    if (group_id) {
      ...
      data.members = safeParseJSON4(data.members, []);
    ...
    if (user_id) {
      ...
      results.forEach((r) => r.members = safeParseJSON4(r.members, []));
      return json(request, env, { success: true, data: results });
    }
...
async function onRequestPatch3(context) {
  ...
  try {
    ...
    let members = safeParseJSON4(group.members, []);
...
function safeParseJSON4(str, fallback) {
...
__name(safeParseJSON4, "safeParseJSON4");
__name2(safeParseJSON4, "safeParseJSON");
  ...
  try {
    if (noticeId) {
      ...
      const [likeCount, commentsResult] = await Promise.all([
        env.DB.prepare("SELECT COUNT(*) as cnt FROM notice_likes WHERE notice_id = ?").bind(noticeId).first(),
        env.DB.prepare("SELECT * FROM notice_comments WHERE notice_id = ? ORDER BY created_at ASC").bind(noticeId).all()
      ]);
      return json(request, env, {
        success: true,
        data: { ...notice, like_count: likeCount?.cnt || 0, comments: commentsResult?.results || [] }
      });
    }
    const { results } = await env.DB.prepare(
      'SELECT * FROM notices WHERE is_hidden = 0 ORDER BY CASE WHEN type = "important" THEN 0 ELSE 1 END, created_at DESC'
    ).all();
    return json(request, env, { success: true, data: results }, { headers: LIST_RESPONSE_HEADERS });
  } catch (err) {
...
__name(uniqueStrings4, "uniqueStrings4");
__name2(uniqueStrings4, "uniqueStrings");
function parseClassIds(value) {
  if (Array.isArray(value)) return uniqueStrings4(value);
  if (typeof value !== "string") return [];
  const trimmed = value.trim();
  if (!trimmed) return [];
  try {
    const parsed = JSON.parse(trimmed);
    return Array.isArray(parsed) ? uniqueStrings4(parsed) : [];
  } catch {
    return [];
  }
}
__name(parseClassIds, "parseClassIds");
__name2(parseClassIds, "parseClassIds");
async function onRequest19(context) {
  ...
  try {
    ...
    if (!folders || folders.length === 0) {
      return json(request, env, { success: true, data: [] }, { headers: RESPONSE_HEADERS4 });
    }
    const allClassIds = uniqueStrings4(folders.flatMap((folder) => parseClassIds(folder.class_ids)));
    ...
    const enrichedFolders = folders.map((folder) => {
      ...
      const folderClasses = classIds.map((id) => {
        const classData = classMap.get(String(id));
        if (!classData) {
...
async function onRequestOptions37(context) {
  return options(context.request, context.env);
}
...
async function onRequestOptions38(context) {
  return options(context.request, context.env);
}
...
function normalizeBoolean3(value) {
  if (typeof value === "boolean") return value;
  if (typeof value === "number") return value === 1;
  if (typeof value === "string") return ["1", "true", "yes", "on"].includes(value.toLowerCase());
  return false;
}
__name(normalizeBoolean3, "normalizeBoolean3");
__name2(normalizeBoolean3, "normalizeBoolean");
  ...
  try {
    ...
    const users = (results || []).map((user) => {
      ...
      return {
        ...
        is_blacklisted: normalizeBoolean3(normalized.is_blacklisted),
...
async function onRequestOptions40(context) {
  return options(context.request, context.env);
}
...
async function onRequest22(context) {
  ...
  if (method === "GET") {
    ...
    try {
      ...
      return json(request, env, { success: true, data: enriched, meta: { limit, offset, count: enriched.length } }, { headers: RESPONSE_HEADERS6 });
    } catch (error) {
...
// .wrangler/tmp/bundle-ZUN7Or/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ZUN7Or/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5124223222267719.js.map
