...
// .wrangler/tmp/bundle-UDfpQr/checked-fetch.js
...
// .wrangler/tmp/pages-6q3u3x/functionsWorker-0.5703094196829178.mjs
...
async function refreshClassStats(db, classId) {
  ...
  const [
    ...
    db.prepare("SELECT COUNT(*) AS total_enrollments FROM enrollments WHERE class_id = ?").bind(id).first().catch(() => ({ total_enrollments: 0 })),
    db.prepare("SELECT COALESCE(AVG(rating), 0) AS avg_rating, COUNT(*) AS review_count FROM reviews WHERE class_id = ?").bind(id).first().catch(() => ({ avg_rating: 0, review_count: 0 })),
    db.prepare("SELECT COUNT(*) AS bookmark_count FROM class_bookmarks WHERE class_id = ?").bind(id).first().catch(() => ({ bookmark_count: 0 })),
    ...
    db.prepare(`
      ...
      FROM user_passes
      WHERE class_id = ?
    `).bind(id).first().catch(() => ({ total_passes_issued: 0, total_passes_used: 0 }))
  ]);
...
async function loadEnrollmentStats(db, classId) {
  const row = await db.prepare(`
    SELECT
      COUNT(DISTINCT CASE
    ...
    FROM enrollments
    WHERE class_id = ?
  `).bind(classId).first().catch(() => ({ recent_active_students: 0 }));
  return {
    recent_active_students: Number(row?.recent_active_students || 0)
  };
}
__name(loadEnrollmentStats, "loadEnrollmentStats");
__name2(loadEnrollmentStats, "loadEnrollmentStats");
  ...
  try {
    ...
    if (request.method === "GET") {
      ...
      const [
        ...
        loadEnrollmentStats(db, classId).catch(() => ({ recent_active_students: 0 })),
        loadRevenueByRange(db, classId).catch(() => ({ day: 0, week: 0, month: 0, year: 0 })),
        loadMeetingStats(db, classId).catch(() => ({ recent_attendance_count: 0, recent_attendee_count: 0 })),
        loadRefundSummary(db, classId).catch(() => ({ total_refund_amount: 0 })),
        loadRecentRefundLogs(db, classId).catch(() => [])
      ]);
      const totalStudents = Number(classRow.total_enrollments || classRow.current_participants || 0);
      const recentActiveCount = Number(enrollmentStats?.recent_active_students || 0);
...
function extractClassIds(rawFolder) {
  ...
  return uniqueStrings2([...directClassIds, ...snakeClassIds, ...objectIds]);
}
...
function parseDeletedFolderIds(body) {
  ...
  return uniqueStrings2(parseMaybeJsonArray(raw));
}
...
async function deleteFolders(db, type, folderIds) {
  const ids = uniqueStrings2(folderIds);
...
function parseClassIds(value) {
  if (Array.isArray(value)) return uniqueStrings3(value);
  if (typeof value !== "string") return [];
  const trimmed = value.trim();
  if (!trimmed) return [];
  try {
    const parsed = JSON.parse(trimmed);
    return Array.isArray(parsed) ? uniqueStrings3(parsed) : [];
  } catch {
    return [];
  }
}
...
async function onRequest19(context) {
  ...
  try {
    ...
    const allClassIds = uniqueStrings3(folders.flatMap((folder) => parseClassIds(folder.class_ids)));
...
// .wrangler/tmp/bundle-UDfpQr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-UDfpQr/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5703094196829178.js.map
