...
export async function ensureAuthSchema(db) {
  ...
  await addColumnIfMissing(db, 'sessions', 'auth_provider TEXT');
  await addColumnIfMissing(db, 'sessions', 'auth_provider_user_id TEXT');
  ...
  `).run();
  await db.prepare('CREATE UNIQUE INDEX IF NOT EXISTS idx_social_accounts_provider_user ON social_accounts(provider, provider_user_id)').run();
  await db.prepare('CREATE UNIQUE INDEX IF NOT EXISTS idx_social_accounts_user_provider ON social_accounts(user_id, provider)').run();
